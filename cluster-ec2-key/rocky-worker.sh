...
# hostnamectl set-hostname ${HOSTNAME}.ocs.internal

useradd -m sge_admin
echo "sge_admin ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/sge_admin

mkdir -p /home/sge_admin/.ssh
cp /home/ec2-user/.ssh/authorized_keys /home/sge_admin/.ssh/
...
sudo -u sge_admin /opt/ocs/install_execd \
  -auto \
  -qmaster master01.ocs.internal \
