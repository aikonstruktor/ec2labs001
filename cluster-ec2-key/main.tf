...
resource "aws_instance" "devbox" {
  ...
  provisioner "file" {
    source      = "~/.ssh/id_rsa_ocs"
    destination = "/home/${each.value.ami_user}/.ssh/id_rsa_ocs"
  }

  provisioner "file" {
    source      = "~/.ssh/id_rsa_ocs.pub"
    destination = "/home/${each.value.ami_user}/.ssh/id_rsa_ocs.pub"
  }
  ...
  provisioner "file" {
    source      = "./${each.value.ami_user}-${each.value.role}.sh"
    destination = "/home/${each.value.ami_user}/${each.value.ami_user}-${each.value.role}.sh"
  }

  provisioner "remote-exec" {
    inline = [
      "hostnamectl set-hostname ${each.value.name}.ocs.internal",
      ...
      "/home/${each.value.ami_user}/${each.value.ami_user}-${each.value.role}.sh",
    ]
  }
}
...
resource "aws_route53_record" "devbox_records" {
  for_each = aws_instance.devbox

  zone_id = aws_route53_zone.ocs.zone_id
  name    = "${each.key}.${var.private_domain}"
  type    = "A"
  ttl     = 60
